...
{
    // NobleCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // NobleCoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // NobleCoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // NobleCoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // NobleCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // NobleCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // NobleCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // NobleCoin: increase each by adding 2 to bitcoin's value.
...
void static NobleCoinMiner(CWallet *pwallet)
{
    printf("NobleCoinMiner started\n");
    ...
    {
        printf("NobleCoinMiner terminated\n");
        throw;
    }
}
